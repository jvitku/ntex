...
/**
 * this converts tabulator specified hierarchical structure of the document into
 * a LaTeX itemization
 * 
...
public class Itemization {
	...
	private static final String[] itemEnders = { "\\\\(sub)*section\\{",
			"\\\\begin\\{liter" };
	...
	/**
	 * In order to not to add v-spaces at the end of chapters we want to
	 * identify whether the white line is at the end of the chapter
	 * 
	 * @param lines
	...
	private boolean willEnd(String[] lines, int actual) {
		...
		for (int i = actual + 1; i < lines.length - 1; i++) {
			...
			tmp = lines[i].replaceAll("\\S", "x"); // replace all
													// non-white-spaces with x
	...
	/**
	 * determine whether the actual one is the last (non-white) line in the
	 * document
	 * 
	 * @param lines
	 * @param actual
	...
	/**
	 * check whether there is some itemization breaker (e.g. \section{..) if so,
	 * close all actual itemization and return the result
	 * 
	 * @param line
	 * @return result
	 */
	private String handleItemEnders(String[] lines, int which) {
		...
		// if there is some itemization ender (or the EOF will follow), decrease
		// itemization to 0
	...
	private String checkDocumentEnd(String[] lines, int which) {
		...
		if (this.documentWillEnd(lines, which) && this.itemizing) {
			line = this.decreaseByAfterLine(lines[which],
					this.actualItemDepth + 1);
			this.itemizing = false; // no itemization initialized
		}
		return line;
	...
	private Marker countTabs(String line) {
		 ...
		 * System.out.println("Commentdetecteeeeeeeeeeed"); return new
		 * Marker(0,false, false, true, line); }
		 */
		...
		for (int i = 0; i < line.length(); i++) {
			if (line.charAt(i) != '\t') {
				if (line.charAt(i) == '-') {
					...
					return new Marker(i, true, false, false,
							line.substring(i + 1));
				} else {
	...
	private class Marker {
		...
		public Marker(int nTabs, boolean item, boolean free, boolean comment,
				String line) {
