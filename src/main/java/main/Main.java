...
public class Main {
	...
	public static final AltPaths ap = new AltPaths("app/"); // where to (also) look for files
	...
	// here is the list of custom commands, convert all "FROM" custom ones "TO"
	// LaTeX ones..
	...
	public static void main(String[] args) {
		...
		BufferedReader ntex, temp;
		BufferedWriter wr;
		
		try {
			ntex = Files.getReader(m.notes, ap,log);
			temp = Files.getReader(m.template, ap,log);
			wr = Files.getWriter(m.out, log);
		} catch (Exception e) {
			return;
		}
		// parse the input ntex file, if no \{title} tak found, append also file
		// name
	...
	private boolean parseArguments(String[] args, Logger log) {
		...
		while (poc < args.length) {
			if (args[poc].equalsIgnoreCase("-name")) {
				...
				log.err("Unrecognized parameter: " + args[poc] + " "
						+ args[poc + 1]);
				this.printUsage(log);
				return false;
			}
	...
	private void printUsage(Logger log) {
		log.err("Usage: If no argument found, I am looking for files: '"
				+ defName + ".ntex' and '" + template
	...
	private void conclusion(Logger log) {
		...
		log.log("=============================================================");
		if (log.getErrors() > 0)
			log.log("Compilation failed: " + log.getErrors()
					+ " errors found. PDF may, or may not be created..");
		else {
	...
	public void parse(String name, BufferedReader ntex, BufferedReader temp,
		...
		try {
			// parse the abstract
			t.copyAbstract(Files.getReader(notes,ap, log), wr);
			// get entire -ntex- text
			lines = t.getTheRest(Files.getReader(notes,ap, log));
